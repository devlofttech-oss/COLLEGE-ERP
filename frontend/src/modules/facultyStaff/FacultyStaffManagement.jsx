    ...
    <div className="fixed inset-0 z-[90] flex items-center justify-center bg-[#071e27]/50 p-4 backdrop-blur-sm">
      <div className={cx('max-h-[92vh] w-full overflow-hidden rounded-2xl border border-white/35 bg-[#f3faff]/90 shadow-[0_30px_90px_rgba(7,30,39,.22)] backdrop-blur-2xl', maxWidth)}>
        <div className="flex items-start justify-between border-b border-white/35 px-6 py-5">
          <div>
            <p className="text-[11px] font-bold uppercase text-[#006a62]">Faculty & Staff</p>
  ...
  if (staffMember?.photoUrl) {
    return (
      <img
        ...
        alt=""
        className={cx(size, 'shrink-0 rounded-2xl object-cover ring-1 ring-white/50')}
      />
    );
  }
  return (
    <div className={cx(size, 'flex shrink-0 items-center justify-center rounded-2xl bg-[#004d4d] font-bold text-white shadow-[0_10px_24px_rgba(0,77,77,.18)]')}>
    ...
    <form onSubmit={submit}>
      ...
      >
        <div className="max-h-[62vh] space-y-6 overflow-y-auto p-6">
          <section>
            <h3 className="mb-3 text-sm font-bold text-[#003434]">Profile</h3>
            <div className="grid gap-4 sm:grid-cols-2">
              <label>
                <span className="mb-1.5 block text-xs font-bold text-[#3f4848]">Employee ID</span>
              ...
              <label>
                <span className="mb-1.5 block text-xs font-bold text-[#3f4848]">Phone *</span>
              ...
              <label>
                <span className="mb-1.5 block text-xs font-bold text-[#3f4848]">Department</span>
              ...
              <label>
                <span className="mb-1.5 block text-xs font-bold text-[#3f4848]">Linked Role</span>
          ...
          <section>
            <h3 className="mb-3 text-sm font-bold text-[#003434]">Photo & Address</h3>
            <div className="grid gap-4 sm:grid-cols-2">
              <label>
                <span className="mb-1.5 block text-xs font-bold text-[#3f4848]">Photo File</span>
                <input
                  type="file"
              ...
              <label>
                <span className="mb-1.5 block text-xs font-bold text-[#3f4848]">Photo URL</span>
                <input value={form.photoUrl} onChange={(event) => update('photoUrl', event.target.value)} className={inputClass} />
              </label>
              <label>
                <span className="mb-1.5 block text-xs font-bold text-[#3f4848]">Photo Key</span>
    ...
    <form onSubmit={submit}>
      ...
      >
        <div className="grid gap-4 p-6 sm:grid-cols-2">
          ...
          <label>
            <span className="mb-1.5 block text-xs font-bold text-[#3f4848]">Code</span>
            <input value={form.code} onChange={(event) => update('code', event.target.value)} className={inputClass} />
          </label>
          <label>
            <span className="mb-1.5 block text-xs font-bold text-[#3f4848]">Head Staff</span>
    ...
    <form onSubmit={submit}>
      ...
      >
        <div className="grid gap-4 p-6">
          ...
          <label>
            <span className="mb-1.5 block text-xs font-bold text-[#3f4848]">Password *</span>
            <input type="password" value={form.password} onChange={(event) => update('password', event.target.value)} className={inputClass} />
          </label>
          <label>
            <span className="mb-1.5 block text-xs font-bold text-[#3f4848]">Backend Role *</span>
            <select value={form.role} onChange={(event) => update('role', event.target.value)} className={inputClass}>
              {ROLE_OPTIONS.map((role) => <option key={role.id} value={role.id}>{role.label}</option>)}
            </select>
          </label>
    ...
    return (
      <div className="erp-staff-page min-w-0">
        <section className="erp-glass-card rounded-2xl p-8 text-center">
          <h1 className="font-['Montserrat'] text-2xl font-bold text-[#003434]">Faculty & Staff</h1>
          <p className="mt-2 text-sm font-semibold text-[#3f4848]">You do not have permission to view staff records.</p>
        </section>
      </div>
    );
  }
  ...
  return (
    <div className="erp-staff-page min-w-0">
      <div className="mb-7 flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
        <div>
          <div className="mb-2 flex items-center gap-2 text-[11px] font-bold uppercase text-[#3f4848]">
            ...
            <span>/</span>
            <span className="text-[#006a62]">Staff</span>
          </div>
          <h1 className="font-['Montserrat'] text-3xl font-bold text-[#003434]">Faculty & Staff</h1>
          <p className="mt-2 text-sm text-[#3f4848]">Backend-backed staff records, departments, documents, and login creation.</p>
          {loadError && <p className="mt-2 text-xs font-semibold text-rose-600">{loadError}</p>}
        </div>
        <div className="flex flex-wrap gap-3">
          <button type="button" onClick={loadStaff} className="inline-flex h-11 items-center justify-center gap-2 rounded-xl border border-white/45 bg-white/40 px-4 text-sm font-bold text-[#004d4d]">
            <RefreshCcw size={17} /> Refresh
          </button>
          {canCreate && (
            <button type="button" onClick={() => setStaffModalRecord(null)} disabled={saving} className="inline-flex h-11 items-center justify-center gap-2 rounded-xl bg-[#004d4d] px-5 text-sm font-bold text-white shadow-[0_14px_30px_rgba(0,77,77,.2)] disabled:bg-slate-300">
      ...
      <section className="mb-6 grid gap-4 lg:grid-cols-5">
        {[
          ['Loaded Staff', count, <Users key="staff-icon" size={18} className="text-[#006a62]" />],
          ['Active', summary.active, <BadgeCheck key="active-icon" size={18} className="text-[#006a62]" />],
          ['Teaching', summary.teaching, <UserRound key="teaching-icon" size={18} className="text-[#006a62]" />],
          ['Non-teaching', summary.nonTeaching, <Building2 key="non-teaching-icon" size={18} className="text-[#006a62]" />],
          ['Linked Logins', summary.linked, <KeyRound key="login-icon" size={18} className="text-[#006a62]" />],
      ...
      <section className="erp-glass-card mb-6 rounded-2xl p-5">
        <div className="grid gap-4 lg:grid-cols-12">
          <label className="lg:col-span-4">
            <span className="mb-1.5 block text-xs font-bold text-[#3f4848]">Search Loaded Records</span>
            <span className="relative block">
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-[#6f7978]" size={17} />
              <input value={filters.q} onChange={(event) => updateFilter('q', event.target.value)} className="h-11 w-full rounded-xl border border-white/40 bg-white/45 pl-10 pr-4 text-sm text-[#071e27] outline-none focus:border-[#006a62] focus:ring-4 focus:ring-[#66d9cc]/20" placeholder="Name or employee ID" />
            </span>
          </label>
          <label className="lg:col-span-2">
            <span className="mb-1.5 block text-xs font-bold text-[#3f4848]">Type</span>
            <select value={filters.type} onChange={(event) => updateFilter('type', event.target.value)} className="h-11 w-full rounded-xl border border-white/40 bg-white/45 px-3 text-sm text-[#071e27] outline-none focus:border-[#006a62]">
          ...
          <label className="lg:col-span-2">
            <span className="mb-1.5 block text-xs font-bold text-[#3f4848]">Department</span>
            <select value={filters.departmentId} onChange={(event) => updateFilter('departmentId', event.target.value)} className="h-11 w-full rounded-xl border border-white/40 bg-white/45 px-3 text-sm text-[#071e27] outline-none focus:border-[#006a62]">
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_360px]">
        <section className="erp-glass-card overflow-hidden rounded-2xl">
          <div className="flex items-center justify-between border-b border-white/35 px-5 py-4">
            <h2 className="text-sm font-bold text-[#003434]">Staff Records</h2>
            <span className="rounded-full bg-white/45 px-3 py-1 text-[11px] font-bold text-[#3f4848]">{count} backend record{count === 1 ? '' : 's'}</span>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full min-w-[880px] text-sm">
              ...
              <tbody className="divide-y divide-white/30">
                {loading && (
                  <tr>
                    <td colSpan="6" className="px-5 py-10 text-center text-sm font-semibold text-[#3f4848]">
                      <Loader2 className="mr-2 inline animate-spin" size={16} /> Loading staff...
                    </td>
                  </tr>
                )}
                {!loading && staffMembers.map((staffMember) => (
                  <tr key={staffMember.id} className="cursor-pointer hover:bg-white/25" onClick={() => openStaff(staffMember)}>
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-3">
                        <StaffAvatar staffMember={staffMember} />
                        <div>
                          <p className="font-bold text-[#071e27]">{staffMember.name || '-'}</p>
                          <p className="mt-1 text-xs font-semibold text-[#3f4848]">{staffMember.employeeId || staffMember.id}</p>
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="px-5 py-4">
                      <p className="font-semibold text-[#071e27]">{departmentNameFor(staffMember, departmentMap)}</p>
                      <p className="mt-1 text-xs text-[#3f4848]">{valueOrDash(staffMember.designation)}</p>
                    </td>
                    <td className="px-5 py-4 font-semibold text-[#3f4848]">{typeLabel(staffMember.type)}</td>
                    <td className="px-5 py-4">
                    ...
                    <td className="px-5 py-4">
                      <div className="flex justify-end gap-2" onClick={(event) => event.stopPropagation()}>
                        <button type="button" onClick={() => openStaff(staffMember)} className="inline-flex h-9 w-9 items-center justify-center rounded-xl bg-white/45 text-[#004d4d]" aria-label="View staff">
                          <Eye size={15} />
                        </button>
                        {canEdit && (
                          <button type="button" onClick={() => setStaffModalRecord(staffMember)} className="inline-flex h-9 w-9 items-center justify-center rounded-xl bg-white/45 text-[#004d4d]" aria-label="Edit staff">
                            <Edit3 size={15} />
                          </button>
                        )}
                        {canArchive && (
                          <button type="button" onClick={() => archiveOrRestoreStaff(staffMember)} className="inline-flex h-9 w-9 items-center justify-center rounded-xl bg-white/45 text-[#004d4d]" aria-label={staffMember.archived ? 'Restore staff' : 'Archive staff'}>
                ...
                {!loading && !staffMembers.length && (
                  <tr>
                    <td colSpan="6" className="px-5 py-12 text-center text-sm font-semibold text-[#3f4848]">No staff records found.</td>
                  </tr>
                )}
              </tbody>
            </table>
        ...
        <aside className="space-y-5">
          <section className="erp-glass-card rounded-2xl p-5">
            <div className="mb-4 flex items-center justify-between gap-3">
              <div>
                <p className="text-[11px] font-bold uppercase text-[#3f4848]">Departments</p>
                <h2 className="text-lg font-bold text-[#003434]">Backend Records</h2>
              </div>
              {canCreate && (
                <button type="button" onClick={() => setDepartmentModalRecord(null)} className="inline-flex h-9 w-9 items-center justify-center rounded-xl bg-[#004d4d] text-white" aria-label="Add department">
                  <Plus size={16} />
                </button>
              )}
            </div>
            <div className="space-y-3">
              {departments.map((department) => {
                ...
                return (
                  <div key={department.id} className="rounded-xl border border-white/35 bg-white/35 p-4">
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <p className="font-bold text-[#071e27]">{department.name}</p>
                        <p className="mt-1 text-xs text-[#3f4848]">{department.code || 'No code'} | Head: {head?.name || department.headStaffId || '-'}</p>
                      </div>
                    ...
                    <div className="mt-3 flex gap-2">
                      {canEdit && (
                        <button type="button" onClick={() => setDepartmentModalRecord(department)} className="inline-flex h-8 items-center gap-2 rounded-lg bg-white/55 px-3 text-xs font-bold text-[#004d4d]">
                          <Edit3 size={13} /> Edit
                        </button>
                      )}
                      {canArchive && (
                        <button type="button" onClick={() => archiveSelectedDepartment(department)} className="inline-flex h-8 items-center gap-2 rounded-lg bg-white/55 px-3 text-xs font-bold text-[#004d4d]">
              ...
              {!departments.length && <div className="rounded-xl border border-dashed border-[#bfc8c8] bg-white/35 p-6 text-center text-sm font-semibold text-[#3f4848]">No departments found.</div>}
            </div>
          </section>
        </aside>
      </div>
      ...
      {selectedStaff && (
        <>
          <button type="button" aria-label="Close staff details" onClick={closeDrawer} className="fixed inset-0 z-[70] bg-[#071e27]/30 backdrop-blur-sm" />
          <aside className="fixed right-0 top-0 z-[80] flex h-screen w-full max-w-xl flex-col overflow-hidden bg-[#f3faff] shadow-2xl">
            ...
            <div className="flex border-b border-[#cfe6f2] bg-white/45 px-4 py-3">
              {[
                ...
                return (
                  <button
                    ...
                    className={cx('mr-2 inline-flex h-10 items-center gap-2 rounded-xl px-4 text-xs font-bold', drawerTab === tab.id ? 'bg-[#004d4d] text-white' : 'bg-white/45 text-[#3f4848]')}
                  >
            ...
            <div className="flex-1 overflow-y-auto">
              {drawerLoading && (
                <div className="m-6 flex items-center gap-2 text-sm font-semibold text-[#3f4848]">
              ...
              {drawerTab === 'documents' && (
                <div className="space-y-4 p-6">
                  <div className="flex items-center justify-between gap-3">
                    <h4 className="text-sm font-bold text-[#003434]">Staff Documents</h4>
                    {canEdit && (
                  ...
                  {!documents.length && <div className="rounded-xl border border-dashed border-[#bfc8c8] bg-white/35 p-8 text-center text-sm font-semibold text-[#3f4848]">No staff documents found.</div>}
                </div>
              )}
            </div>
            ...
            <div className="grid gap-3 border-t border-[#cfe6f2] bg-white/55 p-5 sm:grid-cols-2">
              {canEdit && (
                <button type="button" onClick={() => setStaffModalRecord(selectedStaff)} className="inline-flex h-10 items-center justify-center gap-2 rounded-xl border border-[#cfe6f2] bg-white px-4 text-sm font-bold text-[#004d4d]">
                  <Edit3 size={16} /> Edit
                </button>
              )}
              {canCreate && !selectedStaff.userId && (
                <button type="button" onClick={() => setLoginModalOpen(true)} className="inline-flex h-10 items-center justify-center gap-2 rounded-xl bg-[#004d4d] px-4 text-sm font-bold text-white">
                  <KeyRound size={16} /> Create Login
                </button>
              )}
              {canArchive && (
                <button type="button" onClick={() => archiveOrRestoreStaff(selectedStaff)} className="inline-flex h-10 items-center justify-center gap-2 rounded-xl border border-[#cfe6f2] bg-white px-4 text-sm font-bold text-[#004d4d]">
