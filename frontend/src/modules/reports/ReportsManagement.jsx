...
function ReportCard({ active, report, onSelect }) {
  return (
    <button
      ...
      className={`erp-report-category-card min-h-32 rounded-2xl border p-4 text-left transition ${
        active ? 'is-active border-[#006a62] bg-white/60 shadow-[0_18px_42px_rgba(7,30,39,.12)]' : 'border-white/35 bg-white/35 hover:bg-white/50'
      }`}
    >
      <span className="flex items-start justify-between gap-3">
        <span className="erp-report-category-icon flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-white/45 text-[#006a62]">
          <FileText size={18} />
        </span>
        <span className="erp-report-category-pill rounded-full bg-white/45 px-3 py-1 text-[11px] font-bold uppercase text-[#006a62]">
          {report.group}
        </span>
      </span>
      <span className="mt-4 block text-sm font-bold text-[#003434]">{report.label}</span>
      <span className="mt-1 block text-xs font-semibold text-[#3f4848]">{report.name}</span>
      <span className="mt-3 block text-xs font-semibold text-[#3f4848]">{report.description}</span>
    </button>
  );
}
  ...
  return (
    <section className="erp-glass-card rounded-2xl p-5">
      <div className="mb-4 flex items-center justify-between gap-3">
        <div>
          <h2 className="text-sm font-bold text-[#003434]">Filters</h2>
          <p className="mt-1 text-xs font-semibold text-[#3f4848]">{fields.length ? 'Report parameters.' : 'No parameters for this report.'}</p>
        </div>
        <SlidersHorizontal size={19} className="text-[#006a62]" />
      </div>
      <form
      ...
      >
        {fields.map((field) => {
          ...
          return (
            <label key={field}>
              <span className="mb-1.5 block text-xs font-bold text-[#3f4848]">{meta.label}</span>
              <input
                value={filters[field] || ''}
...
function SummaryPanel({ summary }) {
  ...
  return (
    <section className="erp-glass-card rounded-2xl p-5">
      <h2 className="mb-4 text-sm font-bold text-[#003434]">Summary</h2>
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        {entries.map(([key, value]) => (
          <div key={key} className="rounded-xl bg-white/40 p-4">
            <div className="text-[11px] font-bold uppercase text-[#3f4848]">{labelize(key)}</div>
            <div className="mt-2 text-lg font-bold text-[#003434]">{formatReportValue(key, value)}</div>
          </div>
        ))}
      </div>
    </section>
  ...
  return (
    <section className="erp-glass-card overflow-hidden rounded-2xl">
      <div className="flex flex-col gap-3 border-b border-white/35 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h2 className="text-sm font-bold text-[#003434]">Result</h2>
          <p className="mt-1 text-xs font-semibold text-[#3f4848]">{normalized.rows.length} rows and {normalized.columns.length} columns.</p>
        </div>
      ...
      {loading ? (
        <div className="p-8 text-center text-sm font-semibold text-[#3f4848]">
          ...
          <table className="w-full min-w-[900px] text-sm">
            ...
            <tbody>
              {normalized.rows.map((row, rowIndex) => (
                <tr key={row.id || rowIndex} className="border-t border-white/30">
                  {normalized.columns.map((column) => (
                    <td key={`${row.id || rowIndex}-${column}`} className="px-5 py-4 font-semibold text-[#3f4848]">
  ...
  return (
    <div className="erp-reports-page">
      <div className="flex flex-col gap-4 pb-6 lg:flex-row lg:items-center lg:justify-between">
        <div>
          <p className="text-[11px] font-bold uppercase text-[#006a62]">Reports</p>
          <h1 className="mt-1 font-['Montserrat'] text-3xl font-bold text-[#003434]">Report Center</h1>
          <p className="mt-2 text-sm font-semibold text-[#3f4848]">Operational reporting by module.</p>
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
          ...
          {canExport && (
            <button
              ...
              className="inline-flex h-11 items-center justify-center gap-2 rounded-xl bg-[#004d4d] px-4 text-sm font-bold text-white disabled:opacity-70"
            >
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <SummaryCard icon={<FileText size={20} className="text-[#006a62]" />} label="Reports" loading={loadingList} value={reports.length} />
        <SummaryCard icon={<BarChart3 size={20} className="text-[#006a62]" />} label="Rows" loading={running} value={resultSummary.rows} />
        <SummaryCard icon={<SlidersHorizontal size={20} className="text-[#006a62]" />} label="Columns" loading={running} value={resultSummary.columns} />
        <SummaryCard icon={<Download size={20} className="text-[#006a62]" />} label="Summary Fields" loading={running} value={resultSummary.summaryFields} />
      </div>

      <div className="mt-5 grid gap-5 xl:grid-cols-[360px_minmax(0,1fr)]">
        <section className="erp-glass-card rounded-2xl p-4">
          <div className="mb-4 flex items-center justify-between gap-3 px-1">
            <h2 className="text-sm font-bold text-[#003434]">Available Reports</h2>
            <span className="rounded-full bg-white/45 px-3 py-1 text-[11px] font-bold uppercase text-[#006a62]">{reports.length}</span>
          </div>
          <div className="grid max-h-[720px] gap-3 overflow-y-auto pr-1">
            {loadingList && (
              <div className="p-6 text-center text-sm font-semibold text-[#3f4848]">
          ...
          {selectedReport && (
            <section className="erp-glass-card rounded-2xl p-5">
              <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                <div>
                  <p className="text-[11px] font-bold uppercase text-[#006a62]">{selectedReport.group}</p>
                  <h2 className="mt-1 font-['Montserrat'] text-2xl font-bold text-[#003434]">{selectedReport.label}</h2>
                  <p className="mt-2 text-sm font-semibold text-[#3f4848]">{selectedReport.description}</p>
                </div>
                <span className="w-fit rounded-full border border-[#81f3e5]/60 bg-[#81f3e5]/35 px-3 py-1 text-[11px] font-bold uppercase text-[#006f66]">
                  {selectedReport.name}
                </span>
              </div>
            </section>
