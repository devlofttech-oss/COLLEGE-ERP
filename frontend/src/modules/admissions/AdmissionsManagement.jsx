    ...
    <div className="fixed inset-0 z-[90] flex items-center justify-center bg-slate-900/50 p-4">
      <div className={`max-h-[92vh] w-full ${maxWidth} overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-[0_20px_60px_rgba(0,0,0,.15)]`}>
        <div className="flex items-start justify-between border-b border-slate-100 px-6 py-5">
          <div>
            <p className="text-[11px] font-bold uppercase text-brand-500">Admissions</p>
    ...
    <form onSubmit={submit}>
      ...
      >
        <div className="max-h-[62vh] space-y-6 overflow-y-auto p-6">
          <section>
            <h3 className="mb-3 text-sm font-bold text-slate-800">Applicant</h3>
            <div className="grid gap-4 sm:grid-cols-2">
              <label>
                <span className="mb-1.5 block text-xs font-bold text-slate-500">Student Name{!isEdit ? ' *' : ''}</span>
                <input value={form.studentName} onChange={(event) => update('studentName', event.target.value)} className={inputClass} />
              </label>
              <label>
                <span className="mb-1.5 block text-xs font-bold text-slate-500">Parent Name{!isEdit ? ' *' : ''}</span>
                <input value={form.parentName} onChange={(event) => update('parentName', event.target.value)} className={inputClass} />
              </label>
              <label>
                <span className="mb-1.5 block text-xs font-bold text-slate-500">Phone{!isEdit ? ' *' : ''}</span>
              ...
              <label>
                <span className="mb-1.5 block text-xs font-bold text-slate-500">DOB</span>
          ...
          <section>
            <h3 className="mb-3 text-sm font-bold text-slate-800">Interest</h3>
            <div className="grid gap-4 sm:grid-cols-2">
              <label>
              ...
              <label>
                <span className="mb-1.5 block text-xs font-bold text-slate-500">Course Interested{!isEdit ? ' *' : ''}</span>
              ...
              <label>
                <span className="mb-1.5 block text-xs font-bold text-slate-500">Class Interested</span>
              ...
              <label>
                <span className="mb-1.5 block text-xs font-bold text-slate-500">Source</span>
              ...
              <label>
                <span className="mb-1.5 block text-xs font-bold text-slate-500">Follow-up Date</span>
                <input type="date" value={form.followUpDate} onChange={(event) => update('followUpDate', event.target.value)} className={inputClass} />
              </label>
              {isEdit && (
                <>
                  <label>
                    <span className="mb-1.5 block text-xs font-bold text-slate-500">Stage</span>
              ...
              <label className="sm:col-span-2">
                <span className="mb-1.5 block text-xs font-bold text-slate-500">Documents</span>
    ...
    <form onSubmit={submit}>
      ...
      >
        <div className="grid gap-4 p-6">
          <label>
            <span className="mb-1.5 block text-xs font-bold text-slate-500">Note *</span>
            <textarea value={form.note} onChange={(event) => setForm((current) => ({ ...current, note: event.target.value }))} className={`${inputClass} py-3`} rows={4} />
          </label>
          <label>
            <span className="mb-1.5 block text-xs font-bold text-slate-500">Next Follow-up Date</span>
          ...
          <label>
            <span className="mb-1.5 block text-xs font-bold text-slate-500">Outcome</span>
    ...
    <form onSubmit={submit}>
      <ModalFrame
        ...
        footer={(
          <div className="flex justify-end gap-3 border-t border-slate-100 px-6 py-4">
            <button type="button" onClick={onClose} className="h-10 rounded-xl border border-slate-200 bg-slate-100 px-5 text-sm font-bold text-slate-500">Cancel</button>
            <button type="submit" className={`inline-flex h-10 items-center gap-2 rounded-xl px-5 text-sm font-bold text-white shadow-[0_12px_24px_rgba(0,77,77,.18)] ${approving ? 'bg-brand-700' : 'bg-rose-600'}`}>
      ...
      >
        <div className="grid gap-4 p-6">
          {approving ? (
            <>
              <label>
                <span className="mb-1.5 block text-xs font-bold text-slate-500">Admission Number</span>
  ...
  return (
    <div className="erp-admissions-page min-w-0">
      <div className="mb-7 flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
        ...
        {canCreate && (
          <button type="button" onClick={() => setAdmissionModalRecord(null)} disabled={saving} className="inline-flex h-11 items-center justify-center gap-2 rounded-xl bg-brand-700 px-5 text-sm font-bold text-white shadow-[0_14px_30px_rgba(0,77,77,.2)] disabled:bg-slate-300">
      ...
      <section className="mb-6 grid gap-4 lg:grid-cols-5">
        {[
          ['Total Pipeline', summary.total, <UserPlus key="total-icon" size={18} className="text-emerald-600" />],
          ['Enquiry', summary.enquiry, <CircleDot key="enquiry-icon" size={18} className="text-emerald-600" />],
          ['Application', summary.application, <ClipboardCheck key="application-icon" size={18} className="text-emerald-600" />],
          ['Approved', summary.approved, <BadgeCheck key="approved-icon" size={18} className="text-emerald-600" />],
          ['Converted', summary.converted, <GraduationCap key="converted-icon" size={18} className="text-emerald-600" />],
      ...
      <section className="tt-card mb-6 rounded-2xl p-5">
        <div className="grid gap-4 lg:grid-cols-12">
          <label className="lg:col-span-4">
            <span className="mb-1.5 block text-xs font-bold text-slate-500">Filter Loaded Records</span>
            <span className="relative block">
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={17} />
              <input value={filters.q} onChange={(event) => updateFilter('q', event.target.value)} className="h-11 w-full rounded-xl border border-slate-200 bg-[#f8f9fa] pl-10 pr-4 text-sm text-slate-900 outline-none focus:border-brand-500 focus:ring-4 focus:ring-brand-100" placeholder="Name, enquiry no, phone" />
            </span>
          </label>
          ...
          <label className="lg:col-span-2">
            <span className="mb-1.5 block text-xs font-bold text-slate-500">Stage</span>
            <select value={filters.stage} onChange={(event) => updateFilter('stage', event.target.value)} className="h-11 w-full rounded-xl border border-slate-200 bg-[#f8f9fa] px-3 text-sm text-slate-900 outline-none focus:border-brand-500">
          ...
          <label className="lg:col-span-2">
            <span className="mb-1.5 block text-xs font-bold text-slate-500">Source</span>
            <select value={filters.source} onChange={(event) => updateFilter('source', event.target.value)} className="h-11 w-full rounded-xl border border-slate-200 bg-[#f8f9fa] px-3 text-sm text-slate-900 outline-none focus:border-brand-500">
              <option value="">All sources</option>
              {mergedSourceOptions.map((source) => <option key={source} value={source}>{source}</option>)}
            </select>
          </label>
          <label className="lg:col-span-4">
            <span className="mb-1.5 block text-xs font-bold text-slate-500">Course Interested</span>
            <select value={filters.courseInterested} onChange={(event) => updateFilter('courseInterested', event.target.value)} className="h-11 w-full rounded-xl border border-slate-200 bg-[#f8f9fa] px-3 text-sm text-slate-900 outline-none focus:border-brand-500">
          ...
          <div className="flex items-end gap-3 lg:col-span-4">
            ...
            <button type="button" onClick={loadAdmissions} className="flex h-11 w-11 items-center justify-center rounded-xl bg-brand-700 text-white" aria-label="Refresh admissions">
              <RefreshCcw size={17} />
            </button>
          </div>
          <div className="flex items-end lg:col-span-4">
            <p className="text-sm font-semibold text-slate-500">{count} backend record{count === 1 ? '' : 's'} loaded.</p>
          </div>
        </div>
      </section>
            ...
            <div key={stage.id} className="flex w-80 shrink-0 flex-col gap-4">
              <div className="flex items-center justify-between px-2">
                <div className="flex items-center gap-2">
                  ...
                  <span className="rounded bg-slate-100 px-2 py-0.5 text-[11px] font-bold text-slate-500">{rows.length}</span>
                </div>
              ...
              <div className="erp-admissions-column flex max-h-[62vh] flex-col gap-4 overflow-y-auto pr-1">
                {loading && (
                  <div className="tt-card rounded-xl p-5 text-sm font-semibold text-slate-500">
                    <Loader2 className="mr-2 inline animate-spin" size={16} /> Loading...
                  </div>
                )}
                {!loading && rows.map((admission) => (
                  <button key={admission.id} type="button" onClick={() => openAdmission(admission)} className="tt-card w-full rounded-xl p-4 text-left">
                    ...
                    <h4 className="font-bold text-slate-800">{admission.studentName || '-'}</h4>
                    <p className="mt-1 text-sm text-slate-500">{admission.courseInterested || admission.classInterested || '-'}</p>
                    <div className="mt-4 flex items-center justify-between gap-3">
                      <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-brand-700 text-[11px] font-bold text-white">
                        {initialsFor(admission.studentName)}
                      </div>
                      <span className="text-[11px] font-semibold text-slate-500">{admission.enquiryNumber || admission.admissionNumber || admission.id}</span>
                    </div>
                  </button>
                ))}
                {!loading && !rows.length && (
      ...
      {selectedAdmission && (
        <>
          <button type="button" aria-label="Close admission details" onClick={closeDrawer} className="fixed inset-0 z-[70] bg-slate-900/20" />
          <aside className="fixed right-0 top-0 z-[80] flex h-screen w-full max-w-xl flex-col overflow-hidden bg-white shadow-2xl">
            ...
            <div className="flex-1 overflow-y-auto p-6">
              ...
              <section className="mb-6">
                <div className="mb-3 flex items-center justify-between gap-3">
                  <h3 className="text-sm font-bold text-slate-800">Follow-ups</h3>
                  {canEdit && (
                    <button type="button" onClick={() => setFollowupModalOpen(true)} className="inline-flex h-9 items-center gap-2 rounded-xl bg-brand-700 px-3 text-xs font-bold text-white">
                ...
                <div className="space-y-3">
                  {followups.map((followup) => (
                    <div key={followup.id} className="rounded-xl border border-slate-100 bg-white p-4">
                      <p className="font-semibold text-slate-900">{followup.note}</p>
            ...
            <div className="grid gap-3 border-t border-slate-100 bg-white p-5 sm:grid-cols-2">
              {canEdit && (
                <button type="button" onClick={() => setAdmissionModalRecord(selectedAdmission)} className="inline-flex h-10 items-center justify-center gap-2 rounded-xl border border-slate-100 bg-white px-4 text-sm font-bold text-brand-700">
                  <Edit3 size={16} /> Edit
                </button>
              )}
              {canMoveSelected && (
                <button type="button" onClick={() => handleMoveToApplication(selectedAdmission)} className="inline-flex h-10 items-center justify-center gap-2 rounded-xl bg-brand-700 px-4 text-sm font-bold text-white">
                  <MoveRight size={16} /> To Application
                </button>
              )}
              {canDecideSelected && (
                <>
                  <button type="button" onClick={() => setDecisionMode('approve')} className="inline-flex h-10 items-center justify-center gap-2 rounded-xl bg-brand-700 px-4 text-sm font-bold text-white">
              ...
              {canConvertSelected && (
                <button type="button" onClick={() => setConvertModalOpen(true)} className="inline-flex h-10 items-center justify-center gap-2 rounded-xl bg-brand-700 px-4 text-sm font-bold text-white">
                  <GraduationCap size={16} /> Convert
                </button>
              )}
              {canEdit && (
                <button type="button" onClick={() => archiveOrRestore(selectedAdmission)} className="inline-flex h-10 items-center justify-center gap-2 rounded-xl border border-slate-100 bg-white px-4 text-sm font-bold text-brand-700">
